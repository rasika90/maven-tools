...
public abstract class AbstractUUFMojo extends AbstractAssemblyMojo {
     ...
     * AbstractAssemblyMojo does not allow child classes to plug-in a custom AssemblyReader. Hence needed to
     * reimplement the below method.
    ...
    public void createOsgiImportsConfig() throws MojoExecutionException {
        ...
        Path osgiImportsConfig = uufOsgiConfigOutDirectory.resolve("osgi-imports.yaml");
        StringBuilder content = new StringBuilder();
        content.append("#This file is auto-generated by UUF Maven Plugin.\n");
        content.append("#Use <import.package> property on pom.xml to configure OSGi imports.\n");
        content.append("Imports:\n");
        for (String importLine : getOsgiImports()) {
            content.append("\t- " + importLine.trim().replaceAll("\\r|\\n", "") + "\n");
        }
        try {
            Files.write(osgiImportsConfig, content.toString().getBytes());
        } catch (IOException e) {
            throw new MojoExecutionException("Error creating file: " + osgiImportsConfig, e);
        }
    }
